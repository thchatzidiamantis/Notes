...
\usepackage[textwidth=2.65cm,textsize=small,colorinlistoftodos]{todonotes} %leave comments in the file
%% N.R. notes
\newcommand{\nrnote}[1]{\todo[color=blue!40,linecolor=green!40!black,size=\tiny]{#1}}
\newcommand{\nrmpar}[1]{\todo[noline,color=blue!40,linecolor=green!40!black,
  size=\tiny]{#1}}
\newcommand{\nrnoteil}[1]{\todo[inline,color=blue!40,linecolor=green!40!black,size=\normalsize]{#1}}

%% T.C. notes: change color to something you like.
\newcommand{\tcnote}[1]{\todo[color=orange!40,linecolor=green!40!black,size=\tiny]{#1}}
\newcommand{\tcmpar}[1]{\todo[noline,color=orange!40,linecolor=green!40!black,
  size=\tiny]{#1}}
\newcommand{\tcnoteil}[1]{\todo[inline,color=orange!40,linecolor=green!40!black,size=\normalsize]{#1}}

%% F.D. notes: change color to something you like.
\newcommand{\fdnote}[1]{\todo[color=blue!40,linecolor=green!40!black,size=\tiny]{#1}}
\newcommand{\fdmpar}[1]{\todo[noline,color=blue!40,linecolor=green!40!black,
  size=\tiny]{#1}}
\newcommand{\fdnoteil}[1]{\todo[inline,color=blue!40,linecolor=green!40!black,size=\normalsize]{#1}}
...
\tcnoteil{$\lambda$ notation for functions. Maybe an example of an inductive type.}
...
\begin{itemize}
    ...
    \begin{align*}
        \Gamma \; \mathrm{ctx} & & \Gamma \vdash A\; \mathrm{type} & & \Gamma \vdash a : A & & \Gamma \vdash A \equiv A' \; \mathrm{type} & & \Gamma \vdash a \equiv a' : A 
    \end{align*}
\end{itemize}   
\end{defin}
...
These fall into one of five categories: \tcnote{add descriptions and examples. What about the generic elements rule in Rijke's book?}
\begin{enumerate} 
...
We also take a hierarchy of universes $\ \mathcal{U}_i, \ i\in\bN$, with the extra assumption that the universes are \textit{cumulative}, i.e., a type in a universe is also in the next one. In formal terms, we have two rules:
\begin{align*}
...
\tcnoteil{Type formation, introduction, elimination and computation rules. Empty, product and coproduct and non-dependent function types. Compare to category theory. Talk about substitution!}

\subsection*{Translating logic, Part 1}
\tcnoteil{Short section of correspondence of product/coproduct/function types and logical operators. Predicates as $A\to \mathcal{U}$}
...
\tcnoteil{I don't know where to put this in the notes: A proposition is a type $A$ with a term of type $$\mathsf{isProp}(A):\equiv\prod_{x,y:A}x=_Ay$$ i.e., $A$ is empty or contractible.}
...
\end{thm}    \tcnote{In the full notes: talk about the universal properties implied by funext.}
...
\tcnoteil{Propositions, sets, ``universal property" of the propositional truncation. Infinite level of truncations, related to homotopy theory.  Note that types like isSet and isEquiv are propositions. AC and LEM related to truncation levels.  Univalence contradicts UIP. If this takes me more than 10 minutes to write I have failed as a logician.}
\tcnoteil{UP of prop. truncation: $A$ type, $f:A\to P$, $P$ proposition s.t. for any other proposition $Q$ the precomposition map (w/$f$) is an equivalence of the function types.}
